const Amizade = require("../models/jogador")
...
class AmizadesController {
  ...
  create(req, res) {
    ...
    let jogadorId = AmizadesDAO.criar(jogador);

    // Faz o response para o browser
    if (jogadorId)
      res.status(201).json({ jogador: AmizadesDAO.buscarPorId(jogadorId) })
    else
  ...
  list(req, res) {
    ...
    let listaJogadores = AmizadesDAO.listar().slice()
  ...
  show(req, res) {
    ...
    let jogador = AmizadesDAO.buscarPorId(parseInt(id));
  ...
  update(req, res) {
    ...
    if (jogador) {
      ...
      AmizadesDAO.atualizar(id, jogador)
      // Cria uma nova variável que recebe a versão verbosa de jogador
      let jogadorVerbose = jogador.verbose()
      // Faz o response para o browser
      res.status(200).json({ jogador: jogadorVerbose });
